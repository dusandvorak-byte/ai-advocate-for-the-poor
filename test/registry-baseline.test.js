...
test('the test-count answer preserves distinct checks and does not invent a cumulative total', () => {
  ...
  assert.equal(current.build, 'v3.2');
  assert.equal(current.distinctAutomatedChecks, 200);
  assert.equal(current.submissionChecks, 54);
  assert.equal(current.postSubmissionChecksAdded, 146);
  assert.deepEqual(current.latestCompleteRun, { passed: 200, failed: 0 });
  assert.equal(current.minimumDocumentedSuccessfulIndividualExecutions, 1240);
  assert.equal(current.minimumCalculation, '54 + 62 + 66 + 85 + 131 + 144 + 144 + 166 + 188 + 200');
